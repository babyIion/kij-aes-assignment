...
def shift_rows(s):
    # the line zero is not shifted, so we go from line one > two > three
...
def mix_column_indiv(a):
    t = a[0] ^ a[1] ^ a[2] ^ a[3]
    u = a[0]

    a[0] = a[0] ^ xtime(a[0] ^ a[1]) ^ t
    a[1] = a[1] ^ xtime(a[1] ^ a[2]) ^ t
    a[2] = a[2] ^ xtime(a[2] ^ a[3]) ^ t
    a[3] = a[3] ^ xtime(a[3] ^ u) ^ t

def mix_columns(s):
    for i in range(4):
        mix_column_indiv(s[i])

def inv_mix_column_indiv(a):
